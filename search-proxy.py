...
# Linux: python-portable/lib/python3*/site-packages
...
win_site = os.path.join(USB, 'python', 'Lib', 'site-packages')
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_signin(self):
        ...
        try:
            env = os.environ.copy()
            env['OLLAMA_HOME'] = os.path.join(USB, 'ollama')
            proc = subprocess.Popen(
                [os.path.join(USB, 'ollama', 'bin', 'ollama'), 'signin'],
                env=env,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            # Leer output durante 5 segundos buscando la URL
            import time
            url = None
            for _ in range(50):
                ...
                urls = re.findall(r'https?://ollama\.com[^\s]+', line)
                if urls:
                    url = urls[0]
                    break
                time.sleep(0.1)
            if url:
                ...
                self.send_json(200, {'message': 'Ejecuta en terminal: ' + os.path.join(USB, 'ollama/bin/ollama') + ' signin'})
